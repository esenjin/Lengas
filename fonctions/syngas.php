...
function syngas_sync_compute_diff(array $series): ?array {
    ...
    foreach ($fields as $key => $new_value) {
        ...
        if (in_array($key, ['genres', 'categories', 'other_contributors'], true)) {
            ...
            if ($normalize_list($old_cmp) === $normalize_list($new_cmp)) continue;
        }
    ...
    // Vignette : signalée à part (pas dans $fields), et seulement si la
    // série n'a PAS déjà de vignette locale. Les vignettes sur Lengas sont
    // personnalisées par l'utilisateur : celle de Syngas n'a d'intérêt que
    // pour combler une fiche qui n'en a encore aucune, jamais pour remplacer
    // un choix déjà fait localement.
    $local_thumbnail = trim((string)($series['image'] ?? ''));
    if ($local_thumbnail === '' && $mapped['thumbnail_url'] !== '') {
        $diff['thumbnail'] = ['old' => '', 'new' => $mapped['thumbnail_url']];
    }
...
function syngas_sync_save_selected(array &$data, array $selections, array $diffs_by_series): array {
    ...
    foreach ($diffs_by_series as $series_id => $diff_info) {
        ...
        // section 4, note vignette) — et seulement si la série n'a pas déjà
        // de vignette locale. $diff_info['thumbnail_url'] est toujours
        // rempli par syngas_map_to_lengas_fields() dès que Syngas en propose
        // une, indépendamment de sa présence dans le diff affiché à
        // l'utilisateur : on revérifie donc ici l'image actuelle plutôt que
        // de se fier au seul diff, pour ne jamais écraser un choix déjà fait
        // localement.
        $has_local_thumbnail = trim((string)($data[$key]['image'] ?? '')) !== '';
        if (!$has_local_thumbnail && !empty($diff_info['thumbnail_url'])) {
