// ──────────────────────────────────────────────────────────────────────────
// assets/js/admin/tools/incomplete.js — Outil « Séries incomplètes »
//
// Recherche en flux (SSE) des tomes manquants, filtres et tri des
// résultats, ajout des tomes manquants et saisie d'une URL MangaUpdates
// pour les séries sans référence.
// ──────────────────────────────────────────────────────────────────────────
...
function bindIncompleteButtons() {
    ...
    document.querySelectorAll('.add-missing-volume').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            .then(r => r.json())
            .then(d => {
                if (d.success) { showSuccessModal('Tome ajouté avec succès !'); refreshAfterAdd(); }
                else showErrorModal(d.message || "Une erreur est survenue lors de l'ajout du tome.");
            })
    ...
    document.querySelectorAll('.add-all-missing-volumes').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            .then(r => r.json())
            .then(d => {
                if (d.success) { showSuccessModal('Tomes ajoutés avec succès !'); refreshAfterAdd(); }
                else showErrorModal(d.message || "Une erreur est survenue lors de l'ajout des tomes.");
            })
